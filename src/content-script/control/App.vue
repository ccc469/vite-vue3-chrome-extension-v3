<template>
  ...
  >
    <div
      id="dialContent"
      ref="dialContent"
      class="flex-col items-center hidden mb-4 space-y-2"
    >
      <button
        type="button"
...
import { Dial, DialInterface } from 'flowbite'

let dial: DialInterface
const toogle = () => dial.toggle()

onMounted(() => {
  const instance = getCurrentInstance()
  const $parentEl = instance?.refs['dialParent'] as HTMLElement
  const $triggerEl = instance?.refs['dialButton'] as HTMLElement
  const $targetEl = instance?.refs['dialContent'] as HTMLElement
  dial = new Dial($parentEl, $triggerEl, $targetEl)
})
</script>
